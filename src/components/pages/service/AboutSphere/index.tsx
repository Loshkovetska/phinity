...
const AboutSphere = observer(() => {
  ...
  useEffect(() => {
    if (!ContentStore.service.about.list) return
  ...
  }, [ContentStore.service.about.list, heightBlock])
  ...
  useEffect(() => {
    if (window.innerWidth > 1120) {
      ...
      let offset = vectRect.top - bodyRect.top,
        offsetBottom = elemRect.top + heightBlock - vectRect!.height
      ...
      window.addEventListener('scroll', () => {
        if (window.scrollY < offset - (window.innerHeight - vectRect.height) / 2) {
          requestAnimationFrame(() => {
            ;(vect as HTMLElement).style.top = `${56}px`
          })
        }
        if (
          window.scrollY >= offset - (window.innerHeight - vectRect.height) / 2 &&
        ...
        <>
          ...
          <div className="about-service__scroll" ref={ref}>
            <div className="about-service__scroll-area">
              {ContentStore.service.about.list?.map((l: any, id) => (
                <div
